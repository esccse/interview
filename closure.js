...
// 谈谈关于闭包引起内存泄漏的问题
// 内存泄漏就是这块内存被占用 且无法回收掉它 造成资源浪费 除非关闭浏览器进程
// 现代浏览器中使用的标记清楚回收机制 闭包本身是不会引起内存泄漏的 当闭包中的变量确定不再使用时 可以将其手动回收掉 可以将引用闭包的函数回收 
// 也可以暴露外部方法直接回收闭包中的变量
// 而ie9之前的ie使用的js回收机制是引用计数回收机制 如果在dom事件中持有自身dom变量时会导致dom的引用计数至少为1 导致无法回收
...
// dom的操作很容易造成内存泄漏 jquery和vue等框架对dom事件做了优化处理 可以使用框架处理dom事件 不过框架只是帮助我们减少发生内存泄漏的可能 平时开发还是要主要开发规范 还是要多留意
